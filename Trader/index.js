...
class Trader {
    ...
    constructor() {
        ...
        this.isOnSimulationMode = config.app.env === "simulation" || config.app.env === "dev";
    }
    ...
    async buy(account, market, symbol, resource, prices, last_price, forced) {
        ...
        //buy if advice is true or its an forced action
        if (advice || forced) {
        ...
        for (let balance of account.balances) {
            ...
            market.class.lastPrices(balance.symbol).then(function (result) {
                ...
                for (let resource of balance.resources) {
                    ...
                    let amount = resource.amount;
                    let totalAmount = demoBalance;

                    if (resource.final_state === "sell") {
                        totalAmount = totalAmount + amount * result.bid
                    }

                    let message = resource.title + ' you have ' + demoBalance + ' demo balance if you sell all you have, you gonna have ' + totalAmount
    ...
    async closeAlarm(alarmId) {
        //TODO check alarm belongs to user
