...
async function migrate() {
    ...
    await orm.query("CREATE TABLE IF NOT EXISTS "+prefix+"markets (id INTEGER PRIMARY KEY AUTO_INCREMENT, title VARCHAR(255), status BOOLEAN, transaction_fee FLOAT, default_hashed_username VARCHAR(255), default_hashed_secret_key VARCHAR(255), default_hashed_special_key VARCHAR(255), created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP)");
    ...
    await orm.query("CREATE TABLE IF NOT EXISTS "+prefix+"resources (id INTEGER PRIMARY KEY AUTO_INCREMENT, balance_id INTEGER(11), buy_strategy_id INTEGER(11), sell_strategy_id INTEGER(11), title VARCHAR(255), amount FLOAT, demo_balance FLOAT, status BOOLEAN, final_price FLOAT, final_state VARCHAR(255) , wave_length INTEGER, trend_alpha FLOAT, smooth_period INTEGER, forecast_count INTEGER, sell_margin FLOAT, buy_margin FLOAT, short_period INTEGER, long_period INTEGER,  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY(balance_id) REFERENCES "+prefix+"balances (id), FOREIGN KEY(sell_strategy_id) REFERENCES "+prefix+"strategies (id), FOREIGN KEY(buy_strategy_id) REFERENCES "+prefix+"strategies (id))");

    await orm.query("CREATE TABLE IF NOT EXISTS "+prefix+"market_logs (id INTEGER PRIMARY KEY AUTO_INCREMENT, resource_id INTEGER(11), market_id INTEGER(11), order_id INTEGER(11), amount FLOAT, symbol VARCHAR(255), value FLOAT, action VARCHAR(50), created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY(resource_id) REFERENCES "+prefix+"resources (id), FOREIGN KEY(market_id) REFERENCES "+prefix+"markets (id))");
    ...
    await orm.query("CREATE TABLE IF NOT EXISTS "+prefix+"alarms ( id int(11) NOT NULL AUTO_INCREMENT, resource_id int(11) NOT NULL, max float DEFAULT NULL, min float DEFAULT NULL, title varchar(45) DEFAULT NULL, created_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, status int(11) NOT NULL DEFAULT '0', last_message_time double DEFAULT NULL, message_count int(11) NOT NULL DEFAULT '0', next_message_time double DEFAULT NULL, PRIMARY KEY (id), KEY fk_auyari_resourceId (resource_id), CONSTRAINT fk_uyari_resourceId FOREIGN KEY (resource_id) REFERENCES _resources (id) ON DELETE NO ACTION ON UPDATE NO ACTION ) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=latin1")
    return true;
}
